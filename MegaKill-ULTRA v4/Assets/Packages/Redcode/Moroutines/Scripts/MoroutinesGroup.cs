...
namespace Redcode.Moroutines
{
    ...
    public class MoroutinesGroup
    {
        ...
        public MoroutinesGroup(params Moroutine[] moroutines)
            : this((IEnumerable<Moroutine>)moroutines) { }
        ...
        {
            return GetUnownedMoroutines(
                State.Reseted | State.Running | State.Stopped | State.Completed | State.Destroyed
            );
        }
        ...
        #region Subscribing
        private MoroutinesGroup OnSubscribe(
            ref Action<MoroutinesGroup> ev,
            Action<MoroutinesGroup> action
        )
        {
            ev += action;
            return this;
        ...
        /// <returns>The moroutines group.</returns>
        public MoroutinesGroup OnReseted(Action<MoroutinesGroup> action) =>
            OnSubscribe(ref Reseted, action);
        ...
        /// <returns>The moroutines group.</returns>
        public MoroutinesGroup OnRunning(Action<MoroutinesGroup> action) =>
            OnSubscribe(ref Running, action);
        ...
        /// <param name="action"><inheritdoc cref="OnReseted(Action{Moroutine})"/></param>
        /// <returns>The moroutines group.</returns>
        public MoroutinesGroup OnStopped(Action<MoroutinesGroup> action) =>
            OnSubscribe(ref Stopped, action);
        #endregion
        ...
        public WaitForAll WaitForComplete() =>
            new(Moroutines.Select(m => m.WaitForComplete()).ToList());
        ...
        /// </summary>
        /// <returns><inheritdoc cref="WaitForComplete"/></returns>
        public WaitForAll WaitForDestroy() =>
            new(Moroutines.Select(m => m.WaitForDestroy()).ToList());
        #endregion
