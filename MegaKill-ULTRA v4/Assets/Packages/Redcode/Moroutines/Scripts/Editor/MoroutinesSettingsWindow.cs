...
namespace Redcode.Moroutines.Editor
{
    internal class MoroutinesSettingsWindow : EditorWindow
    {
        ...
        private void OnEnable() =>
            _settings = Resources.Load<MoroutinesSettings>("Redcode/Moroutines/Settings");

        private void OnDisable() => _window = null;

        private void OnGUI()
        {
            _settings.HideMoroutinesExecuter = EditorGUILayout.Toggle(
                "Hide Moroutines Executer",
                _settings.HideMoroutinesExecuter
            );
        }
    }
}
