    ...
    public class MoroutinesOwnerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.ObjectField(
                "Script:",
                MonoScript.FromMonoBehaviour((MoroutinesOwner)target),
                typeof(MoroutinesOwner),
                false
            );
            GUI.enabled = true;
            ...
            if (
                GUILayout.Button(
                    showButtonText,
                    GUILayout.Width(
                        EditorStyles.label.CalcSize(new GUIContent(showButtonText)).x + 10f
                    )
                )
            )
            ...
            {
                var moroutine = (Moroutine)
                    _moroutinesProperty.GetArrayElementAtIndex(i).managedReferenceValue;
                ...
                GUILayout.Label(
                    new GUIContent(lastResult, lastResult),
                    EditorStyles.label,
                    columnOptions
                );
                #endregion
