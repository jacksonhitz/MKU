...
public static class FindMissingScriptsRecursively
{
    ...
    private static void FindAndRemoveMissingInSelected()
    {
        ...
        foreach (var gameObject in selected)
        {
            ...
            {
                RecursivePrefabSource(
                    gameObject,
                    prefabs,
                    ref removedComponentsCount,
                    ref removedGameObjectsCount
                );
        ...
        if (removedGameObjectsCount == 0)
            Debug.Log(
                "<color=#32a852>There is no selected game objects with missing scripts.</color>"
            );
        else
            Debug.Log(
                $"<color=#c8d649>Removed {removedComponentsCount} missing scripts from {removedGameObjectsCount} game objects.</color>"
    ...
    private static void RecursivePrefabSource(
        ...
        ref int removedComponentsCount,
        ref int removedGameObjectsCount
    )
    {
