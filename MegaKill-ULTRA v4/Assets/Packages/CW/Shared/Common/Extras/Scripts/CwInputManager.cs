    ...
    public class CwInputManager : MonoBehaviour
    {
        ...
        public struct Axis
        {
            ...
            public float GetValue(float delta)
            {
                ...
                value -=
                    CwInput.GetKeyIsHeld(KeyNegativeAlt) == true ? KeySensitivity * delta : 0.0f;
                value +=
                    CwInput.GetKeyIsHeld(KeyPositiveAlt) == true ? KeySensitivity * delta : 0.0f;
                ...
                if (FingerCount > 0 && fingers.Count == FingerCount)
                {
                    ...
                    switch (FingerGesture)
                    {
                        case AxisGesture.HorizontalDrag:
                            value += GetAverageDeltaScaled(fingers).x * FingerSensitivity * scale;
                            break;
                        case AxisGesture.VerticalDrag:
                            value += GetAverageDeltaScaled(fingers).y * FingerSensitivity * scale;
                            break;
                        case AxisGesture.Twist:
                            value += GetAverageTwistRadians(fingers) * FingerSensitivity;
                            break;
                        case AxisGesture.HorizontalPull:
                            value +=
                                GetAveragePullScaled(fingers).x * FingerSensitivity * delta * scale;
                            break;
                        case AxisGesture.VerticalPull:
                            value +=
                                GetAveragePullScaled(fingers).y * FingerSensitivity * delta * scale;
                            break;
                    }
                }
        ...
        public struct Trigger
        {
            ...
            public bool WentDown(Finger finger)
            {
                ...
                if (
                    ...
                    && CwInput.GetKeyWentDown(UseKey) == true
                )
                {
                    return true;
                }
            ...
            public bool IsDown(Finger finger)
            {
                ...
                if (
                    UseKey != KeyCode.None
                    && finger.Index == HOVER_FINGER_INDEX
                    && CwInput.GetKeyIsHeld(UseKey) == true
                )
                {
                    return true;
                }
            ...
            public bool WentUp(Finger finger, bool useAnyFinger = false)
            {
                ...
                if (
                    UseKey != KeyCode.None
                    && finger.Index == HOVER_FINGER_INDEX
                    && CwInput.GetKeyWentUp(UseKey) == true
                )
                {
                    return true;
                }
        ...
        [SerializeField]
        private LayerMask guiLayers = 1 << 5;
        ...
        public static List<Finger> GetFingers(
            bool ignoreStartedOverGui = false,
        ...
        public static void EnsureThisComponentExists()
        {
            if (
                ...
                && CwHelper.FindAnyObjectByType<CwInputManager>() == null
            )
            {
        ...
        private static float GetDeltaRadians(
            Finger finger,
            Vector2 referencePoint,
            Vector2 lastReferencePoint
        )
        {
