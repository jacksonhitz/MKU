...
namespace CW.Common
{
    ...
    public class CwGuide_Inspector : Editor
    {
        ...
        {
            var tgt = (CwGuide)target;

            UpdateStyles();

            EditorGUILayout.LabelField("Thank You For Using " + tgt.LongName + "!", headerStyle);
            EditorGUILayout.LabelField(
                "The documentation can be opened below. To understand how this asset works I also recommend you run the example scenes, and read their description text.",
                bodyStyle
            );

            if (
                GUILayout.Button(new GUIContent("Local Documentation", "Open In Web Browser"))
                == true
            )
            {
                System.Diagnostics.Process.Start(
                    Application.dataPath
                        + AssetDatabase.GetAssetPath(tgt.Documentation).Remove(0, "Assets".Length)
                );
            }

            if (
                GUILayout.Button(new GUIContent("Online Documentation", "Open In Web Browser"))
                == true
            )
            {
            ...
            EditorGUILayout.LabelField(
                "If you haven't already, consider rating this asset. It really helps me out!",
                bodyStyle
            );

            if (
                GUILayout.Button(new GUIContent("Rate This Asset", tgt.LongName + " Asset Page"))
                == true
            )
            {
            ...
            EditorGUILayout.LabelField(
                "I've released a range of assets to speed up your project development, check them out:",
                bodyStyle
            );
        ...
        protected override void OnHeaderGUI()
        {
            ...
            GUILayout.BeginHorizontal("In BigTitle");
            {
                ...
                var height = Mathf.Max(
                    titleStyle.CalcHeight(content, EditorGUIUtility.currentViewWidth - iconWidth),
                    iconWidth
                );
