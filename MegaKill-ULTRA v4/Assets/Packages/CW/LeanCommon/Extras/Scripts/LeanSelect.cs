...
namespace Lean.Common
{
    ...
    public class LeanSelect : MonoBehaviour
    {
        ...
        [SerializeField]
        private ReselectType reselect = ReselectType.SelectAgain;
        ...
        public List<LeanSelectable> Selectables
        {
            ...
            {
                if (selectables == null)
                    selectables = new List<LeanSelectable>();
                return selectables;
            }
        }
        ...
        public LeanSelectableEvent OnSelected
        {
            ...
            {
                if (onSelected == null)
                    onSelected = new LeanSelectableEvent();
                return onSelected;
            }
        }
        ...
        public LeanSelectableEvent OnDeselected
        {
            get
            {
                if (onDeselected == null)
                    onDeselected = new LeanSelectableEvent();
                return onDeselected;
            }
        }
                            ...
                            case LimitType.DeselectFirst:
                                {
                                    if (
                                        selectables.Count > 0
                                        && selectables.Count >= maxSelectables
                                    )
                                    {
...
namespace Lean.Common.Editor
{
    ...
    public class LeanSelect_Editor : CwEditor
    {
        ...
        protected override void OnInspector()
        {
            ...
            var select = (LeanSelectable)
                UnityEditor.EditorGUILayout.ObjectField(
                    new GUIContent("Select", "Drop a selectable object here to select it."),
                    null,
                    typeof(LeanSelectable),
                    true
                );
            var deselect = (LeanSelectable)
                UnityEditor.EditorGUILayout.ObjectField(
                    new GUIContent("Deselect", "Drop a selectable object here to deselect it."),
                    null,
                    typeof(LeanSelectable),
                    true
                );
        ...
        {
            if (
                Any(tgts, t => t.OnSelected.GetPersistentEventCount() > 0) == true
            ...
            if (
                Any(tgts, t => t.OnDeselected.GetPersistentEventCount() > 0) == true
                || showUnusedEvents == true
            )
            {
                Draw("onDeselected");
            ...
            if (
                Any(tgts, t => t.OnNothing.GetPersistentEventCount() > 0) == true
                || showUnusedEvents == true
            )
            {
                Draw("onNothing");
