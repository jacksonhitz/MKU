...
namespace Lean.Common
{
    ...
    public class LeanSelectable : MonoBehaviour
    {
        ...
        public bool SelfSelected
        {
            set
            {
                if (selfSelected != value)
                {
                    selfSelected = value;
                    if (value == true)
                        InvokeOnSelected(null);
                    else
                        InvokeOnDeslected(null);
                }
            }
        ...
        public LeanSelectEvent OnSelected
        {
            ...
            {
                if (onSelected == null)
                    onSelected = new LeanSelectEvent();
                return onSelected;
            }
        }
        ...
        public LeanSelectEvent OnDeselected
        {
            get
            {
                if (onDeselected == null)
                    onDeselected = new LeanSelectEvent();
                return onDeselected;
            }
        }
    ...
    public class LeanSelectable_Editor : CwEditor
    {
        ...
        private void DrawSelected()
        {
            ...
            if (Draw("selfSelected") == true)
            {
                Each(
                    tgts,
                    t => t.SelfSelected = serializedObject.FindProperty("selfSelected").boolValue,
                    true
                );
            }
            BeginDisabled();
        ...
        protected virtual void DrawEvents(bool showUnusedEvents)
        {
            if (
                ...
                || Any(tgts, t => t.OnSelected.GetPersistentEventCount() > 0)
            )
            {
                Draw("onSelected");
            }
            ...
            if (
                showUnusedEvents == true
                || Any(tgts, t => t.OnDeselected.GetPersistentEventCount() > 0)
            )
            {
                Draw("onDeselected");
            }
