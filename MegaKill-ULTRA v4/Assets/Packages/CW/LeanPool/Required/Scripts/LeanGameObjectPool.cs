...
namespace Lean.Pool
{
    ...
    public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public static LinkedList<LeanGameObjectPool> Instances =
            new LinkedList<LeanGameObjectPool>();
        ...
        [SerializeField]
        private NotificationType notification = NotificationType.IPoolable;
        ...
        private static Dictionary<GameObject, LeanGameObjectPool> prefabMap =
            new Dictionary<GameObject, LeanGameObjectPool>();
        ...
        public bool DespawnedClonesMatch
        {
            get
            {
                for (var i = despawnedClones.Count - 1; i >= 0; i--)
                {
                    ...
                    if (
                        clone != null
                        && UnityEditor.PrefabUtility.GetCorrespondingObjectFromSource(clone)
                            != prefab
                    )
                    {
                        return false;
        ...
        )
        {
            if (prefab == null)
            {
                if (warnings == true)
                    Debug.LogWarning(
                        "You're attempting to spawn from a pool with a null prefab",
                        this
                    );
                return false;
            }
        ...
        private bool TryDespawnOldest(ref GameObject clone, bool registerDespawned)
        {
            ...
            while (spawnedClonesList.Count > 0)
            {
                ...
                if (warnings == true)
                    Debug.LogWarning(
                        "This pool contained a null spawned clone, did you accidentally destroy it?",
                        this
                    );
            }
        ...
        public void Detach(GameObject clone, bool cleanLinks = true)
        {
            ...
            {
                if (
                    spawnedClonesHashSet.Remove(clone) == true
                    || spawnedClonesList.Remove(clone) == true
                ...
                else
                {
                    if (warnings == true)
                        Debug.LogWarning(
                            "You're attempting to detach a GameObject that wasn't spawned from this pool.",
                            clone
                        );
                }
            }
            ...
            {
                if (warnings == true)
                    Debug.LogWarning("You're attempting to detach a null GameObject", this);
            }
        }
        ...
        public void PreloadOneMore()
        {
            if (prefab != null)
            {
                ...
                if (warnings == true && capacity > 0 && Total > capacity)
                    Debug.LogWarning(
                        "You've preloaded more than the pool capacity, please verify you're preloading the intended amount.",
                        this
                    );
            }
            else
        ...
        private void RegisterPrefab()
        {
            if (prefab != null)
            {
                ...
                {
                    Debug.LogWarning(
                        "You have multiple pools managing the same prefab (" + prefab.name + ").",
                        existingPool
                    );
                }
                else
        ...
        )
        {
            ...
            if (
                ...
                && UnityEditor.PrefabUtility.IsPartOfRegularPrefab(prefab) == true
            )
            {
    ...
    public class LeanGameObjectPool_Editor : CwEditor
    {
        ...
        protected override void OnInspector()
        {
            ...
            if (Draw("prefab", "The prefab this pool controls.") == true)
            {
                Each(
                    ...
                    {
                        t.Prefab = (GameObject)
                            serializedObject.FindProperty("prefab").objectReferenceValue;
                    },
                    true
                );
            }
            ...
            Draw(
                "notification",
                "If you need to perform a special action when a prefab is spawned or despawned, then this allows you to control how that action is performed. None = If you use this then you must rely on the OnEnable and OnDisable messages. SendMessage = The prefab clone is sent the OnSpawn and OnDespawn messages. BroadcastMessage = The prefab clone and all its children are sent the OnSpawn and OnDespawn messages. IPoolable = The prefab clone's components implementing IPoolable are called. Broadcast IPoolable = The prefab clone and all its child components implementing IPoolable are called."
            );
            Draw(
                "strategy",
        ...
        private void DrawClones(string title, bool spawned, bool despawned, string propertyName)
        {
            ...
            EditorGUILayout.LabelField(
                string.Empty,
                GUILayout.Height(EditorGUI.GetPropertyHeight(property))
            );
