...
namespace IngameDebugConsole
{
    ...
    public static class DebugLogConsole
    {
        ...
        private static readonly List<ConsoleMethodInfo> matchingMethods =
            new List<ConsoleMethodInfo>(4);

        // All the parse functions
        private static readonly Dictionary<Type, ParseFunction> parseFunctions = new Dictionary<
            Type,
            ParseFunction
        >()
        {
        ...
        private static readonly Dictionary<Type, string> typeReadableNames = new Dictionary<
            Type,
            string
        >()
        {
        ...
        internal static readonly CompareInfo caseInsensitiveComparer = new CultureInfo(
            "en-US"
        ).CompareInfo;
        ...
        public static void SearchAssemblyForConsoleMethods(Assembly assembly)
        {
            try
            {
                ...
                foreach (Type type in assembly.GetExportedTypes())
                {
                    ...
                    {
                        foreach (
                            ConsoleAttribute consoleAttribute in method.GetCustomAttributes(
                                typeof(ConsoleAttribute),
                                false
                            )
                        )
                        {
        ...
        public static void LogSystemInfo()
        {
            ...
            stringBuilder
                .Append("Persistent Data Path: ")
                .Append(Application.persistentDataPath)
                .Append("\n");
            stringBuilder
                .Append("StreamingAssets Path: ")
                .Append(Application.streamingAssetsPath)
                .Append("\n");
            stringBuilder
                .Append("Temporary Cache Path: ")
            ...
            stringBuilder
                ...
                .Append("\n");
            stringBuilder
                .Append("Max Texture Size: ")
                .Append(SystemInfo.maxTextureSize)
                .Append("\n");
            stringBuilder
                .Append("Max Cubemap Size: ")
        ...
        )
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (
                    parseFunctions.ContainsKey(parameterType)
            ...
            else
            {
                ...
                while (
                    ...
                    commandFirstIndex--;
                while (
                    commandLastIndex < methods.Count - 1
                    && caseInsensitiveComparer.Compare(
        ...
        public static string GetAutoCompleteCommand(string commandStart, string previousSuggestion)
        {
            int commandIndex = FindCommandIndex(
                !string.IsNullOrEmpty(previousSuggestion) ? previousSuggestion : commandStart
            );
            if (commandIndex < 0)
            {
                commandIndex = ~commandIndex;
                return (
                    commandIndex < methods.Count
                    && caseInsensitiveComparer.IsPrefix(
            ...
            if (commandIndex >= 0)
            {
                ...
                while (commandIndex <= commandLastIndex)
                {
                    ...
                    else
                    {
                        ...
                        if (
                            methods[commandIndex].parameterTypes.Length
            ...
            if (matchingMethods.Count == 0)
            {
                ...
                else
                {
                    ...
                    if (parameterCountMismatch)
                        ...
                        stringBuilder
                            .Append("ERROR: can't find command '")
                            .Append(_command)
                            .Append("'. Did you mean:");
        ...
        public static void FetchArgumentsFromCommand(string command, List<string> commandArguments)
        {
            for (int i = 0; i < command.Length; i++)
            {
                ...
                if (delimiterIndex >= 0)
                {
                    ...
                    i =
                        (endIndex < command.Length - 1 && command[endIndex + 1] == ',')
                            ? endIndex + 1
                            : endIndex;
                }
                else
        ...
        private static int FindCommandIndex(string command)
        {
            ...
            {
                int mid = (min + max) / 2;
                int comparison = caseInsensitiveComparer.Compare(
                    command,
                    methods[mid].command,
                    CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace
                );
        ...
        public static bool ParseEnum(string input, Type enumType, out object output)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                if (orIndex < 0)
                    enumStr = input
                        .Substring(i, (andIndex < 0 ? input.Length : andIndex) - i)
                        .Trim();
                else
                    enumStr = input
                        .Substring(i, (andIndex < 0 ? orIndex : Mathf.Min(andIndex, orIndex)) - i)
                        .Trim();
        ...
        public static bool ParseArray(string input, Type arrayType, out object output)
        {
            ...
            IList result = (IList)
                Activator.CreateInstance(arrayType, new object[1] { valuesToParse.Count });
            output = result;
