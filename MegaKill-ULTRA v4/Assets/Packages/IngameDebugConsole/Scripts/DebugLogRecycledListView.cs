...
#pragma warning restore 0649
        ...
        private readonly DynamicCircularBuffer<DebugLogItem> visibleLogItems =
            new DynamicCircularBuffer<DebugLogItem>(32);
        ...
        public void Initialize(
            ...
            DynamicCircularBuffer<DebugLogEntry> entriesToShow,
            DynamicCircularBuffer<DebugLogEntryTimestamp> timestampsOfEntriesToShow,
            float logItemHeight
        )
        {
        ...
        public void SelectAndFocusOnLogItemAtIndex(int itemIndex)
        {
            ...
            float transformComponentCenterYAtBottom =
                transformComponent.sizeDelta.y - viewportHeight * 0.5f;
            float transformComponentTargetCenterY =
        ...
        public void OnBeforeFilterLogs()
        {
            ...
            if (selectedLogEntry != null)
            {
                ...
                scrollDistanceToSelectedLogEntry =
                    indexOfSelectedLogEntry * ItemHeight - transformComponent.anchoredPosition.y;
            }
        }
        ...
        public void OnAfterFilterLogs()
        {
            ...
            if (selectedLogEntry != null)
            {
                ...
                {
                    if (
                        entriesToShow[i] == selectedLogEntry
                        && collapsedOrderOfSelectedLogEntry-- == 0
                    )
                    {
            ...
            else
            {
                ...
                transformComponent.anchoredPosition = new Vector2(
                    0f,
                    newIndexOfSelectedLogEntry * ItemHeight - scrollDistanceToSelectedLogEntry
                );
            }
        }
        ...
        public void OnLogEntriesRemoved(int removedLogCount)
        {
            if (selectedLogEntry != null)
            {
                bool isSelectedLogEntryRemoved = isCollapseOn
                    ? (selectedLogEntry.count == 0)
                    : (indexOfSelectedLogEntry < removedLogCount);
                if (isSelectedLogEntryRemoved)
                    DeselectSelectedLogItem();
                else
                    indexOfSelectedLogEntry = isCollapseOn
                        ? FindIndexOfLogEntryInReverseDirection(
                            selectedLogEntry,
            ...
            if (!manager.IsLogWindowVisible && manager.SnapToBottom)
            {
                ...
                visibleLogItems.TrimStart(
                    Mathf.Clamp(removedLogCount - currentTopIndex, 0, visibleLogItems.Count),
                    poolLogItemAction
                );
            else
            {
                ...
                if (visibleLogItems.Count > 0)
                    removedLogCount =
                        currentTopIndex
                        - FindIndexOfLogEntryInReverseDirection(
                            visibleLogItems[0].Entry,
                            visibleLogItems[0].Index
                        );
            }
            ...
            else
            {
                ...
                float firstVisibleLogItemInitialYPos = visibleLogItems[0]
                    .Transform
                    .anchoredPosition
                    .y;
                for (int i = 0; i < visibleLogItems.Count; i++)
                ...
                if (!manager.SnapToBottom)
                    transformComponent.anchoredPosition = new Vector2(
                        ...
                        Mathf.Max(
                            0f,
                            transformComponent.anchoredPosition.y
                                - (
                                    visibleLogItems[0].Transform.anchoredPosition.y
        ...
        private void CalculateSelectedLogEntryHeight(DebugLogItem referenceItem = null)
        {
            ...
            heightOfSelectedLogEntry = referenceItem.CalculateExpandedHeight(
                selectedLogEntry,
                (timestampsOfEntriesToShow != null)
                    ? timestampsOfEntriesToShow[indexOfSelectedLogEntry]
                    : (DebugLogEntryTimestamp?)null
            );
        }
        ...
        private void UpdateItemsInTheList(bool updateAllVisibleItemContents)
        {
            if (entriesToShow.Count > 0)
            {
                ...
                int newBottomIndex = Mathf.Min(
                    (int)(contentPosBottom / logItemHeight),
                    entriesToShow.Count - 1
                );
                int newTopIndex = Mathf.Clamp(
                    (int)(contentPosTop / logItemHeight),
                    0,
                    newBottomIndex
                );
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            for (
                                int i = 0, count = newBottomIndex - currentBottomIndex;
                                i < count;
                                i++
                            )
                                visibleLogItems.Add(manager.PopLogItem());
        ...
        private void UpdateLogItemContentsBetweenIndices(
            int topIndex,
            int bottomIndex,
            int logItemOffset
        )
