...
namespace IngameDebugConsole
{
    ...
    public class DynamicCircularBuffer<T>
    {
        ...
        public void AddRange(DynamicCircularBuffer<T> other)
        {
            ...
            int otherElementsBeforeWrap = Mathf.Min(
                other.Count,
                other.array.Length - other.startIndex
            );
            ...
            if (elementsBeforeWrap < otherElementsBeforeWrap) // This array wrapped before the other array
                Array.Copy(
                    other.array,
                    other.startIndex + elementsBeforeWrap,
                    array,
                    0,
                ...
                Array.Copy(
                    ...
                    0,
                    array,
                    insertStartIndex + otherElementsBeforeWrap,
            ...
            if (copiedElements < other.Count) // Both arrays wrapped and there's still some elements left to copy
                Array.Copy(
                    other.array,
                    copiedElements - otherElementsBeforeWrap,
                    array,
                    copiedElements - elementsBeforeWrap,
                    other.Count - copiedElements
                );
        ...
        public int RemoveAll<Y>(
            Predicate<T> shouldRemoveElement,
            Action<T, int> onElementIndexChanged,
        ...
        )
        {
            ...
            int i = startIndex,
                newIndex = startIndex,
                endIndex = startIndex + elementsBeforeWrap;
            for (; i < endIndex; i++)
            {
        ...
        public void TrimEnd(int trimCount, Action<T> perElementCallback = null)
        {
            TrimInternal(
                trimCount,
                (startIndex + Count - trimCount) % array.Length,
                perElementCallback
            );
        }
        ...
        private void TrimInternal(int trimCount, int startIndex, Action<T> perElementCallback)
        {
            ...
            {
                for (
                    int i = startIndex, endIndex = startIndex + elementsBeforeWrap;
                    i < endIndex;
                    i++
                )
                {
