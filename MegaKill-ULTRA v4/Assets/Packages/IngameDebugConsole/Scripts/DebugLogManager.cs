...
#pragma warning disable 0649
        ...
        [SerializeField]
        [HideInInspector]
        public InputAction toggleBinding = new InputAction(
            "Toggle Binding",
        ...
        [HideInInspector]
        [Tooltip(
            "If enabled, the console window will continue receiving logs in the background even if its GameObject is inactive. But the console window's GameObject needs to be activated at least once because its Awake function must be triggered for this to work"
        )]
        ...
        private bool receiveInfoLogs = true,
            receiveWarningLogs = true,
            receiveErrorLogs = true,
            receiveExceptionLogs = true;

        [SerializeField]
        [HideInInspector]
        [Tooltip(
            "If enabled, the arrival times of logs will be recorded and displayed when a log is expanded"
        )]
        ...
        [HideInInspector]
        [Tooltip(
            "If the number of logs reach this limit, the oldest log(s) will be deleted to limit the RAM usage. It's recommended to set this value as low as possible"
        )]
        ...
        [HideInInspector]
        [Tooltip(
            "How many log(s) to delete when the threshold is reached (all logs are iterated during this operation so it should neither be too low nor too high)"
        )]
...
#pragma warning restore 0414
        ...
        [HideInInspector]
        [Tooltip(
            "If enabled, on Android and iOS devices with notch screens, the console window's popup won't be obscured by the screen cutouts"
        )]
...
#pragma warning restore 0649
        ...
        private int newInfoEntryCount = 0,
        ...
        public System.Action OnLogWindowShown,
            OnLogWindowHidden;
        ...
        private void Awake()
        {
            ...
            queuedLogEntries = new DynamicCircularBuffer<QueuedDebugLogEntry>(
                Mathf.Clamp(queuedLogLimit, 16, 4096)
            );
            ...
            collapsedLogEntriesMap = new Dictionary<DebugLogEntry, DebugLogEntry>(
                128,
                new DebugLogEntryContentEqualityComparer()
            );
            ...
            {
                collapsedLogEntriesTimestamps = new DynamicCircularBuffer<DebugLogEntryTimestamp>(
                    128
                );
                uncollapsedLogEntriesTimestamps = new DynamicCircularBuffer<DebugLogEntryTimestamp>(
                    256
                );
                timestampsOfLogEntriesToShow = new DynamicCircularBuffer<DebugLogEntryTimestamp>(
                    256
                );
                queuedLogEntriesTimestamps = new DynamicCircularBuffer<DebugLogEntryTimestamp>(
                    queuedLogEntries.Capacity
                );
            }
            ...
            recycledListView.Initialize(
                this,
                logEntriesToShow,
                timestampsOfLogEntriesToShow,
                logItemPrefab.Transform.sizeDelta.y
            );
            ...
            if (!resizeFromRight)
            {
                RectTransform resizeButtonTR = (RectTransform)
                    resizeButton.GetComponentInParent<DebugLogResizeListener>().transform;
                ...
                ((RectTransform)commandInputField.transform).anchoredPosition += new Vector2(
                    resizeButtonTR.sizeDelta.x,
                    0f
                );
            }
            ...
            if (enableSearchbar)
                searchbar
                    ...
                    .onValueChanged.AddListener(SearchTermChanged);
            else
            {
                searchbar = null;
            ...
            snapToBottomButton
                .GetComponent<Button>()
                .onClick.AddListener(() => SnapToBottom = true);
...
#else
                Debug.LogWarning(
                    "Android JNI module must be enabled in Package Manager for \"Receive Logcat Logs In Android\" to work."
                );
#endif
#endif
            }
        ...
        private void LateUpdate()
        {
            ...
            int numberOfLogsToProcess = isLogWindowVisible
                ? queuedLogEntries.Count
                : (queuedLogEntries.Count - queuedLogLimit);
            ...
            if (uncollapsedLogEntries.Count >= maxLogCount)
            {
                ...
                int numberOfLogsToRemove = Mathf.Min(
                    !isLogWindowVisible
                        ? logsToRemoveAfterMaxLogCount
                        : (
                            uncollapsedLogEntries.Count - maxLogCount + logsToRemoveAfterMaxLogCount
                        ),
            ...
            if (newInfoEntryCount > 0 || newWarningEntryCount > 0 || newErrorEntryCount > 0)
            {
                ...
                if (!isLogWindowVisible)
                {
                    ...
                    if (
                        popupVisibility == PopupVisibility.WhenLogReceived
                    ...
                    )
                    {
                        if (
                            (
                                newInfoEntryCount > 0
                                && (popupVisibilityLogFilter & DebugLogFilter.Info)
                                    == DebugLogFilter.Info
                            )
                            || (
                                newWarningEntryCount > 0
                                && (popupVisibilityLogFilter & DebugLogFilter.Warning)
                                    == DebugLogFilter.Warning
                            )
                            || (
                                newErrorEntryCount > 0
                                && (popupVisibilityLogFilter & DebugLogFilter.Error)
                                    == DebugLogFilter.Error
                            )
                        )
                        {
                            popupManager.Show();
                    ...
                    if (searchbar)
                    {
                        ...
                        else
                        {
                            if (searchbar.parent == searchbarSlotTop)
                            {
                                ...
                                logItemsScrollRectTR.sizeDelta =
                                    logItemsScrollRectOriginalSize
                ...
                if (commandInputField.isFocused && commandHistory.Count > 0)
                {
...
#endif
                    {
...
#endif
                        {
                            ...
                            Keyboard.current[Key.DownArrow].wasPressedThisFrame
        ...
        private char OnValidateCommand(string text, int charIndex, char addedChar)
        {
            if (addedChar == '\t') // Autocomplete attempt
            {
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    string autoCompletedCommand = DebugLogConsole.GetAutoCompleteCommand(
                        commandInputFieldAutoCompleteBase,
                        text
                    );
        ...
        public void ReceivedLog(string logString, string stackTrace, LogType logType)
        {
            ...
            QueuedDebugLogEntry queuedLogEntry = new QueuedDebugLogEntry(
                logString,
                stackTrace,
                logType
            );
            ...
            if (queuedLogEntriesTimestamps != null)
            {
                ...
                queuedLogEntryTimestamp = new DebugLogEntryTimestamp(
                    dateTime,
                    lastElapsedSeconds,
                    lastFrameCount
                );
        ...
        private void ProcessQueuedLogs(int numberOfLogsToProcess)
        {
            for (int i = 0; i < numberOfLogsToProcess; i++)
            {
                ...
                lock (logEntriesLock)
                {
                    ...
                    timestamp =
                        queuedLogEntriesTimestamps != null
                            ? queuedLogEntriesTimestamps.RemoveFirst()
                            : dummyLogEntryTimestamp;
                }
        ...
        )
        {
            ...
            bool isEntryInCollapsedEntryList = collapsedLogEntriesMap.TryGetValue(
                logEntry,
                out existingLogEntry
            );
            ...
            if (isCollapseOn && isEntryInCollapsedEntryList)
            {
                ...
                (!isInSearchMode || queuedLogEntry.MatchesSearchTerm(searchTerm))
                && (
                    logFilter == DebugLogFilter.All
                    || (
                        logType == LogType.Log
                        && ((logFilter & DebugLogFilter.Info) == DebugLogFilter.Info)
                    )
                    || (
                    ...
                    || (
                        logType != LogType.Log
                        && logType != LogType.Warning
                        && ((logFilter & DebugLogFilter.Error) == DebugLogFilter.Error)
                    )
                )
            )
            {
        ...
        private void RemoveOldestLogs(int numberOfLogsToRemove)
        {
            ...
            DebugLogEntry logEntryToSelectAndFocus =
                (
                    indexOfLogEntryToSelectAndFocus >= 0
                    && indexOfLogEntryToSelectAndFocus < logEntriesToShow.Count
                )
            ...
            {
                collapsedLogEntries.RemoveAll(
                    shouldRemoveCollapsedLogEntryPredicate,
                    updateLogEntryCollapsedIndexAction,
                    collapsedLogEntriesTimestamps
                );

                if (isCollapseOn)
                    removedLogEntriesToShowCount = logEntriesToShow.RemoveAll(
                        shouldRemoveLogEntryToShowPredicate,
                        null,
                        timestampsOfLogEntriesToShow
                    );
            }
        ...
        private void OnLogEntriesUpdated(
            bool updateAllVisibleItemContents,
        ...
        public void AdjustLatestPendingLog(bool autoExpand, bool stripStackTrace)
        {
            lock (logEntriesLock)
            {
                ...
                if (stripStackTrace) // Omit the latest log's stack trace
                {
                    ...
                    queuedLogEntries[queuedLogEntries.Count - 1] = new QueuedDebugLogEntry(
                        log.logString,
                        string.Empty,
                        log.logType
                    );
        ...
        private void CollapseButtonPressed()
        {
            ...
            collapseButton.color = isCollapseOn
                ? collapseButtonSelectedColor
                : collapseButtonNormalColor;
        ...
        private void RefreshCommandSuggestions(string command)
        {
            ...
            if (commandChanged)
            {
                ...
                DebugLogConsole.GetCommandSuggestions(
                    command,
                    matchingCommandSuggestions,
                    commandCaretIndexIncrements,
                    ref commandInputFieldPrevCommandName,
                    out numberOfParameters
                );
                if (
                    prevCommandName != commandInputFieldPrevCommandName
        ...
        private void FilterLogs()
        {
            ...
            {
                DynamicCircularBuffer<DebugLogEntry> targetLogEntries = isCollapseOn
                    ? collapsedLogEntries
                    : uncollapsedLogEntries;
                DynamicCircularBuffer<DebugLogEntryTimestamp> targetLogEntriesTimestamps =
                    isCollapseOn ? collapsedLogEntriesTimestamps : uncollapsedLogEntriesTimestamps;
        ...
        public string GetAllLogs()
        {
            ...
            sb.Append("Current time: ")
                .AppendLine((System.DateTime.UtcNow + localTimeUtcOffset).ToString("F"));
        ...
        public void GetAllLogs(
            out DynamicCircularBuffer<DebugLogEntry> logEntries,
            out DynamicCircularBuffer<DebugLogEntryTimestamp> logTimestamps
        )
        {
            // Process all pending logs since we want to return "all" logs
            ProcessQueuedLogs(queuedLogEntries.Count);
